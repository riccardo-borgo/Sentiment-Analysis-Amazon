...
@app.route('/')
def api():
    url = request.args.get('https://www.amazon.com/product-reviews/B08D6VD9TR/ref=cm_cr_dp_d_show_all_btm?ie=UTF8&reviewerType=all_reviews&sortBy=recent&pageNumber=1')
    if url:
        data = scrape(url)
        return jsonify(data)
